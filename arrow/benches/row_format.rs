...
use arrow::util::bench_util::{
    create_dict_from_values, create_primitive_array, create_string_array,
...
fn row_bench(c: &mut Criterion) {
    ...
    let values = create_string_array::<i32>(10, 0.);
    ...
    let values = create_string_array::<i32>(30, 0.);
    let dict = create_dict_from_values::<Int32Type>(4096, 0., &values);
    let cols = vec![Arc::new(dict) as ArrayRef];
    do_bench(c, "4096 string_dictionary_low_cardinality(30, 0)", cols);

    let values = create_string_array::<i32>(100, 0.);
    let dict = create_dict_from_values::<Int32Type>(4096, 0., &values);
    let cols = vec![Arc::new(dict) as ArrayRef];
